...
function ShowCard({ show, index }: { show: Show; index: number }) {
  ...
  return (
    <Link href={`/shows/${show.slug}`} style={{ textDecoration: 'none', flex: '0 0 auto', width: '200px' }}>
      ...
      >
        {/* Poster: 2:3 aspect ratio like the reference */}
        <div style={{ position: 'relative', aspectRatio: '2/3', overflow: 'hidden', background: `linear-gradient(145deg, ${g1}40, ${g2}60)` }}>
          {show.posterUrl ? (
            <img
              src={show.posterUrl} alt={show.name}
              style={{
                position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover',
                transform: hovered ? 'scale(1.1)' : 'scale(1)', transition: 'transform 0.5s'
              }}
            />
          ) : (
          ...
          <div style={{
            ...
            background: 'rgba(15,26,15,0.8)', backdropFilter: 'blur(8px)',
            padding: '2px 8px', borderRadius: '4px',
            fontSize: '9px', fontWeight: '900', color: '#ff6b35',
            border: '1px solid rgba(255,107,53,0.3)', textTransform: 'uppercase'
          }}>
        ...
        <div style={{ padding: '12px 14px' }}>
          <h3 style={{
            fontSize: '14px', fontWeight: '700', color: hovered ? '#ff6b35' : 'white',
            whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', transition: 'color 0.2s', marginBottom: '2px'
          }}>
            {show.name}
          </h3>
    ...
    <div>
      ...
      {!user ? (
        <header style={{ position: 'relative', minHeight: '85vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
          ...
          {/* Gradient overlay — fades to bg at bottom */}
          ...
          <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', maxWidth: '820px', padding: '0 24px' }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '5px 16px', borderRadius: '999px', background: 'rgba(57,255,20,0.12)', border: '1px solid rgba(57,255,20,0.25)', marginBottom: '28px' }}>
              <span style={{
                position: 'relative', width: '8px', height: '8px', borderRadius: '50%', background: '#39ff14', display: 'inline-block', flexShrink: 0,
              }} />
              <span style={{ fontSize: '11px', fontWeight: '900', letterSpacing: '2px', color: '#39ff14', textTransform: 'uppercase' }}>
                Live Quizzes Active Now
              </span>
            </div>
      ...
      <section style={{ padding: user ? '0 80px 64px' : '0 0 64px', marginTop: user ? '0' : '-40px', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '24px', padding: !user ? '0 80px' : undefined }}>
          <div>
            <h2 style={{ fontSize: '28px', fontWeight: '800', color: 'white', letterSpacing: '-0.5px' }}>
              {user ? 'Browse Shows' : 'Featured Series'}
            </h2>
            <p style={{ color: '#4a5e4a', marginTop: '4px', fontSize: '14px' }}>
              Join the active quiz arenas for these trending shows
            </p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', paddingRight: !user ? '80px' : undefined }}>
            ...
            <a href="#" style={{ color: '#ff6b35', fontSize: '13px', fontWeight: '700', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '3px', whiteSpace: 'nowrap' }}>
              View all →
            </a>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div
            ...
            style={{
              ...
              padding: !user ? '0 80px 16px' : '0 0 16px',
              scrollbarWidth: 'none',
            }}
          >
